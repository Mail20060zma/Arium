...
class ChatBot:
    """Класс для управления чатом с ИИ"""
    ...
    def __init__(self, api_key="", base_url="https://g4f.space/api/groq/models", model="meta-llama/llama-4-scout-17b-16e-instruct"):
        ...
        logger.info(f"🤖 Чатбот инициализирован. Модель: {self.model}")
    ...
    def send_message(self, user_message):
        """Отправка сообщения и получение ответа (ОБЪЕДИНЁННАЯ ФУНКЦИЯ)"""
        ...
        try:
            ...
            for chunk in response:
                try:
                    ...
                    logger.warning(f"Ошибка при обработке chunk: {e}")
                    continue
            ...
            logger.info("📤 Получение полного ответа для проверки tool_calls...")
            
            full_response_check = self.client.chat.completions.create(
                model=self.model,
                messages=self.history + [{"role": "assistant", "content": full_response}],
                tools=TOOLS_DEFINITIONS,
                timeout=60
            )
            
            assistant_message = full_response_check.choices[0].message
    ...
    def _process_tool_results(self, assistant_response, tool_results):
        """Обработка результатов tool_calls и отправка повторного запроса (РЕКУРСИВНАЯ)"""
        ...
        try:
            ...
            final_response_check = self.client.chat.completions.create(
                model=self.model,
                messages=self.history + [{"role": "assistant", "content": final_response}],
                tools=TOOLS_DEFINITIONS,
                timeout=60
            )
            
            assistant_message_final = final_response_check.choices[0].message
            
            # Рекурсивная обработка новых tool_calls
            if hasattr(assistant_message_final, 'tool_calls') and assistant_message_final.tool_calls:
                logger.info(f"🔄 Обнаружено {len(assistant_message_final.tool_calls)} дополнительных вызовов функций (рекурсия)")
                print(f"\n🔧 ИИ запросил выполнение ещё {len(assistant_message_final.tool_calls)} функций!\n")
                
                # Добавляем финальный ответ в историю
                self.history.append({"role": "assistant", "content": final_response})
                
                # Обрабатываем новые tool calls рекурсивно
                new_tool_results = self.process_tool_calls(assistant_message_final)
...
def main():
    ...
    print("Чат с ассистентом (введите 'выход' для завершения)")
    print("=" * 50)
    logger.info("Чат начинается. Команды: 'выход', 'exit', 'quit'")
